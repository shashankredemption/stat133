...
m <- matrix(rexp(1, rate = 3),nrow= 10,ncol= 10)
...
f <- family[which(family$age>50 & family$gender=="f"),]

  
# [1 pt]
# Create [f2] a subset of family with only men 6 foot tall or more
fm <- family[which(family$height>72 & family$gender=="m"),]

  
# [1 pt]
# Create [f3] a subset of family of people whose name starts with T
f3 <- family[which(substr(family$name,1,1)=='T'),]
...
boxplot(setosa$Sepal.Length, versicolor$Sepal.Length, viriginca$Sepal.Length)
...
points(virginica$Petal.Length, virginica$Petal.Width, col = "black")
