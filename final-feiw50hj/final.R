...
github.name <- "shashankredemption"
email.address <- "sbhargava@berkeley.edu"
...
y <- lapply(1:10, function(x) {seq(from=x, to=5000, by=x)})
...
set.seed(42)
z <- sample(c(TRUE, FALSE), 5000, replace = TRUE, c(.3,.7))
...
# 
t2 <-  table(infants$smoke, infants$number, exclude = if (infants$ed == "College") c (NA, NaN))
...
plot(mtcars$wt, mtcars$disp, xlab = "Weight", ylab = "Engine Displacement", col = as.numeric(mtcars$am))
...
plot(WorldBank$life.expectancy ~ WorldBank$fertility.rate, pch = ".", col = as.numeric(WorldBank$region), ylab = "Life Expectancy", xlab = "Fertility Rate")
abline(v=2.1)
...
plot(WorldBank1960$life.expectancy ~ WorldBank1960$fertility.rate, pch = ".", col = as.numeric(WorldBank$region), ylab = "Life Expectancy in 1960", xlab = "Fertility Rate in 1960")
plot(WorldBank2013$life.expectancy ~ WorldBank2013$fertility.rate, pch = ".", col = as.numeric(WorldBank$region),  ylab = "Life Expectancy in 2013", xlab = "Fertility Rate in 2013")
par(mfrow = c(1,1))
# [4 pts]
...
WorldBankIDA <- WorldBank[WorldBank$lending == "IDA", ]
hist(WorldBankIDA$GDP.per.capita.Current.USD, main="GDP Per Capita for IDA Lending Countries", xlab = "GDP Per Capita")
...
maxforentry <- function(x) {
  maxval <- 0
  for (i in 1:length(x)) {
    if (i < length(x)) {
      maxval <- max(maxval, abs(diff(c(x[i], x[i+1]))))
    }
  }
  return(maxval)
}
max.diff.rain <- lapply(rain, maxforentry)
...
prop.rain <- lapply(rain, raindays)
...
standardizeVar <- function(m, cols=TRUE){
   x <- matrix(nrow = nrow(m), ncol = ncol(m))
   if (cols) {
     for (i in 1:ncol(m)) {
       x[i, ] <- m[i, ] - mean(m[i, ]) / sd(m[i, ])
     }
   }
   else {
     for (i in 1:ncol(m)) {
...
NumJackpot <- function(k, B) {
  n.jackpot<- vector()
  for (j in 1:B) {
    winning <- sort(sample(1:19, 3))
    counter <- 0
    for (i in 1:k) {
      ticket <- sort(sample(1:19, 3))
      if (ticket[1] == winning[1] & ticket[2] == winning[2] & ticket[3] == winning[3]) {
        counter = counter + 1
      }
    }
    n.jackpot[j] <- counter
...
truevalues <- sapply(lengths/3, is.integer)
dna2 <- dna[truevalues]
